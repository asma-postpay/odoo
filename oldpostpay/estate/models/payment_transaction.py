...
class PaymentTransaction(models.Model):
    ...
    def action_postpay_set_done(self):
        """ Set the state of the postpay transaction to 'done'.
        ...
        self._handle_notification_data('postpay', notification_data)

    def action_postpay_set_canceled(self):
        """ Set the state of the postpay transaction to 'cancel'.
    ...
    def action_postpay_set_error(self):
        """ Set the state of the postpay transaction to 'error'.
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError("postpay: " + _("The transaction is not linked to a token."))

        simulated_state = self.token_id.postpay_simulated_state
    ...
    def _send_refund_request(self, **kwargs):
        ...
        refund_tx._handle_notification_data('postpay', notification_data)
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        tx._handle_notification_data('postpay', notification_data)
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        if provider_code != 'postpay' or len(tx) == 1:
            return tx

        reference = notification_data.get('reference')
        tx = self.search([('reference', '=', reference), ('provider_code', '=', 'postpay')])
        if not tx:
            raise ValidationError(
                "postpay: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_notification_data(self, notification_data):
        ...
        self.provider_reference = f'postpay-{self.reference}'
        ...
        if self.tokenize:
            ...
            self._postpay_tokenize_from_notification_data(notification_data)
        ...
        if state == 'pending':
            ...
            self._set_error(_("You selected the following postpay payment status: %s", state))

    def _postpay_tokenize_from_notification_data(self, notification_data):
        ...
        token = self.env['payment.token'].create({
            ...
            'postpay _simulated_state': state,
        })
        self.write({
            'token_id': token,
            'tokenize': False,
